...
def post_comment(article_id: str, author: str, body: str) -> bool:
    ...
    try:
        comment_id = hashlib.md5(
            f"{article_id}:{author}:{time.time()}".encode()
        ).hexdigest()[:16]
        ...
        pipe = r.pipeline()
        pipe.hset(f"comment:{comment_id}", mapping={k: json.dumps(v) if isinstance(v, dict) else str(v) for k, v in comment.items()})
        pipe.rpush(f"comments:{article_id}", json.dumps(comment))
        pipe.execute()
        return True
    except Exception as e:
